...
from .coco_utils import fast_eval_recall, results2json, segm2json
...
class DistEvalmAPHook(DistEvalHook):
    ...
    def evaluate(self, runner, results):
        json_results = segm2json(self.dataset, results)
        coco_dets = self.dataset.coco.loadRes(json_results[1])
        img_ids = self.dataset.coco.getImgIds()
        cocoEval = COCOeval(self.dataset.coco, coco_dets, 'segm')
        cocoEval.params.imgIds = img_ids
        cocoEval.evaluate()
        cocoEval.accumulate()
        cocoEval.summarize()
        runner.log_buffer.output['stats'] = cocoEval.stats
