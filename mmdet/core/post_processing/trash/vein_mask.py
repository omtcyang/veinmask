...
def get_indexdeeps(start, end, centerpoint, params, inf):
    ...
    if phiDelta < max(angle_range, 720 / params['num_directions']):
        return idxDeeps
        ...
        if not is_deep:
            ...
            if judge_deep(thetasPhiDeep, thetasPhiCur, thetasPhiNxt, vein_length_0, vein_length_1, distance_threshold):
                ...
                if nt == num_tails - 1:
                    # 再迭代一次，求新的中心点坐标
                    # pre
                    centerpoint_xp = int(vein_length * 0.5 * math.cos(theta_cur) + centerpoint_x)
                    centerpoint_yp = int(vein_length * 0.5 * math.sin(theta_cur) + centerpoint_y)
                    # 计算当前位置下，指定方向下与轮廓的交点坐标

                    # 增加clip----------------------------
                    centerpoint_xp, centerpoint_yp = clip((centerpoint_xp, centerpoint_yp), vein.shape[2] - 1, vein.shape[1] - 1)
                    # 增加clip----------------------------

                    vein_length_p = vein[:, centerpoint_yp, centerpoint_xp][nxtNum]

                    # new center
                    centerpoint_xc = int(vein_length_p * 0.5 * math.cos(theta_nxt) + centerpoint_xp)
                    centerpoint_yc = int(vein_length_p * 0.5 * math.sin(theta_nxt) + centerpoint_yp)
                    centerpoint_xc, centerpoint_yc = clip((centerpoint_xc, centerpoint_yc), vein.shape[2] - 1, vein.shape[1] - 1)
                    centerpoint_c = [centerpoint_xc, centerpoint_yc]
                    vein_length_p1 = vein[:, centerpoint_yc, centerpoint_xc][curNum]

                    thetasPhiDeep1 = point_phi(centerpoint, centerpoint_c)
                    if thetasPhiDeep1 == 180:
                        thetasPhiDeep1 = -180

                    # ！bug！当原始中心点和深度搜索的中心点角度在一定范围内，并且深度搜索的中心点不会在目标轮廓线上时，视为有效！bug！
                    if judge_deep(thetasPhiDeep1, thetasPhiCur, thetasPhiNxt, vein_length_p, vein_length_p1, distance_threshold):
                        centers.append(centerpoint_c)
                        # 计算当前位置下，指定方向下与轮廓的交点坐标
