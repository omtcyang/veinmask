...
def multiclass_nms_with_mask(multi_bboxes,
    ...
    nms_type = nms_cfg_.pop('type', 'nms')
    nms_op = getattr(nms_wrapper, nms_type)
    multi_masks = np.array(multi_masks)
    for i in range(1, num_classes):
        cls_inds = multi_scores[:, i] > score_thr
        if not cls_inds.any():
            continue
        ...
        if multi_bboxes.shape[1] == 4:
            _bboxes = multi_bboxes[cls_inds, :]
            _masks = multi_masks[cls_inds.cpu().numpy()]
        else:
            _bboxes = multi_bboxes[cls_inds, i * 4:(i + 1) * 4]
        _scores = multi_scores[cls_inds, i]
        if score_factors is not None:
    ...
    if bboxes:
        ...
        if bboxes.shape[0] > max_num:
            ...
            bboxes = bboxes[inds]
            labels = labels[inds]
            masks = np.array(masks)[inds.cpu().numpy()]
        else:
            masks = np.array(masks)
        # # 增加不同类之间的nms
        # bboxes, inds = nms_op(bboxes, iou_thr=0.7)
        # labels = labels[inds.cpu().numpy()]
        # masks = masks[inds.cpu().numpy()]
    else:
        bboxes = multi_bboxes.new_zeros((0, 5))
        labels = multi_bboxes.new_zeros((0,), dtype=torch.long)
        masks = np.array([])
