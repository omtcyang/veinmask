...
from ..Fusion.SCCSFusion import SCCSFusion
...
def distance2mask(topk_inds, distances, points, stride=8, scale_factor=(2, 1.7286)):
    ...
    mask = np.concatenate([co[:, :, np.newaxis], si[:, :, np.newaxis]], axis=-1)
    return mask


@HEADS.register_module
class VeinMask_Head(nn.Module):
    ...
    def __init__(self,
                 ...
                 distance=125):
        super(VeinMask_Head, self).__init__()
    ...
    def _init_layers(self):
        ...
        self.SCCSFusion = SCCSFusion(self.in_channels)
    ...
    def loss(self,
        ...
        # ground truth-------------------------------------------
        labels, sample_points, bbox_targets, mask_targets, centroidness = self.polar_target(all_level_points, extra_data)
        # ground truth-------------------------------------------
        ...
        flatten_mask_preds = [mask_pred.permute(0, 2, 3, 1).reshape(-1, line) for mask_pred in mask_preds]
        ...
        if num_pos > 0:
            ...
            flatten_points = torch.cat([points.repeat(num_imgs, 1) for points in all_level_points])  # [num_pixel,2]
            ...
            loss_mask = self.loss_mask(pos_mask_preds,
                                       ...
                                       weight=pos_centerness_targets,
                                       avg_factor=pos_centerness_targets.sum())
            loss_sample_points = self.loss_sample_point(pos_mask_sample_point_preds, pos_mask_sample_point_targets)
    ...
    def polar_target(self, points, extra_data):
        ...
        # split to per imgs, per level
        num_points = [center.size(0) for center in points]
        labels_list = [labels.split(num_points, 0) for labels in labels_list]
        sample_points_list = [sample_points.split(num_points, 0) for sample_points in sample_points_list]
        bbox_targets_list = [bbox_targets.split(num_points, 0) for bbox_targets in bbox_targets_list]
        mask_targets_list = [mask_targets.split(num_points, 0) for mask_targets in mask_targets_list]
        centroidness_list = [centroidness.split(num_points, 0) for centroidness in centroidness_list]
        ...
        for i in range(num_levels):
            ...
            concat_lvl_sample_points.append(torch.cat([sample_points[i] for sample_points in sample_points_list]))
            concat_lvl_bbox_targets.append(torch.cat([bbox_targets[i] for bbox_targets in bbox_targets_list]))
            concat_lvl_mask_targets.append(torch.cat([mask_targets[i] for mask_targets in mask_targets_list]))
            concat_lvl_centroidness.append(torch.cat([centroidness[i] for centroidness in centroidness_list]))
    ...
    def get_bboxes(self,
        ...
        for img_id in range(len(img_metas)):
            ...
            mask_pred_list = [mask_preds[i][img_id].detach() for i in range(num_levels)]
    ...
    def get_bboxes_single(self,
        ...
        for cls_score, bbox_pred, mask_pred, centerness, points in zip(cls_scores, bbox_preds, mask_preds, centernesses, mlvl_points):
            ...
            # masks = distance2maskUpsample(topk_inds, mask_pred_copy, points_copy, self.strides[i], scale_factor, self.distance)
            masks = distance2mask(topk_inds, mask_pred_copy, points_copy, self.strides[i], scale_factor)
