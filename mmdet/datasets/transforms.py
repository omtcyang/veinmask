...
#
#     def __call__(self, imgs, scale, flip=False, keep_ratio=True):
#         if keep_ratio:
#             imgs, scale_factor = mmcv.imrescale(imgs, scale, return_scale=True)
#         else:
#             imgs, w_scale, h_scale = mmcv.imresize(imgs, scale, return_scale=True)
#             scale_factor = np.array([w_scale, h_scale, w_scale, h_scale], dtype=np.float32)
#         img_shape = imgs.shape
#         imgs = mmcv.imnormalize(imgs, self.mean, self.std, self.to_rgb)
#         if flip:
#             imgs = mmcv.imflip(imgs)
#         if self.size_divisor is not None:
#             imgs = mmcv.impad_to_multiple(imgs, self.size_divisor)
#             pad_shape = imgs.shape
#         else:
#             pad_shape = img_shape
#         imgs = imgs.transpose(2, 0, 1)
#         return imgs, img_shape, pad_shape, scale_factor
...
class ImageTransform(object):
    ...
    def __call__(self, img, scale, flip=False, keep_ratio=True):
        ...
        if self.size_divisor is not None:
            ...
            img = mmcv.impad(img, shape=(896, 1408), pad_val=0)
            pad_shape = img.shape
