...
class Coco_Seg_Dataset(CustomDataset):
    ...
    def load_annotations(self, ann_file):
        ...
        self.count = {0: 0,
                      1: 112 * 176,
                      2: 112 * 176 + 56 * 88,
                      3: 112 * 176 + 56 * 88 + 28 * 44,
                      4: 112 * 176 + 56 * 88 + 28 * 44 + 14 * 22}
        return img_infos
    ...
    def polar_target_single(self, gt_bboxes, masks, gt_labels, points, regress_ranges):
        ...
        for p, id in zip(pos_inds, pos_mask_ids):
            ...
            if temple2[int(y), int(x)] == 0:
                labels[p] = 0
                continue
            dists = self.get_num_coordinates(x, y, pos_mask_contour, line)
    ...
    def get_single_centerpoint(self, mask):
        ...
        if not (0 <= center[1] < mask.shape[0] and 0 <= center[0] < mask.shape[1]):
            if len(contour) > 1:
                ...
                cv2.drawContours(mask_temp, [contour[0]], 0, 1, -1)
            else:
                mask_temp = mask
            center = self.get_center(mask_temp)
            flag = 1
    ...
    def random_sample_within_mask(self, pid, x, y, pos_mask_contour, mask_targets, strides, sample_point_labels, temple):
        ...
        dist = self.get_num_coordinates(x, y, pos_mask_contour, 2 * line)
        ...
        for s_point in sample_point:
            ...
            temp_dist = self.get_num_coordinates(x, y, pos_mask_contour, line)
            mask_targets[self.count[ceil] + index] = temp_dist

    def get_ceiling(self, pid):
        if pid < 19712:
            return 0
        elif pid < 112 * 176 + 56 * 88:
            return 1
        elif pid < 112 * 176 + 56 * 88 + 28 * 44:
            return 2
        elif pid < 112 * 176 + 56 * 88 + 28 * 44 + 14 * 22:
            return 3
        return 4
